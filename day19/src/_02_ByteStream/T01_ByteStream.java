...
public class T01_ByteStream {
	...
	public static void main(String[] args) {
		/*
		 ...
		 반드시 그 외부매체와 통로를 만들어야됨. 이게 바로 스트링
		 
		 * 스트링(통로)의 특징
		  - 단방향 : 입력 혹은 출력 하나만 가능
		  	=> 입력과 출력을 동시에 하고자 한다면 => 입력용 스트림/출력용 스트림을 따로 열어야 됨
		  - 선입선출(FIFO) : 통로가 파이프같으 개념이기 때문에 먼저 들어 오는 값이 먼저 나감.
		  - 시간지연(delay)라는 문제가 생길 수 있음
		  
		   * 스트림의 구분
		   	> 통로의 사이즈 (1byte짜리 / 2byte짜리)
		   		- 바이트 스트림 : 1byte짜리만 왔다 갔다 할 수 있는 정도의 사이즈 => 입력(InputStream) / 중력(OutputStream)
		   		- 문자 스트림 : 2byte짜리 왔다 갔다 할 수 있는 정도의 사이즈 => 입력(Reader) / 출력(Writer)
		   	> 외부매체와 직접연결 유무
